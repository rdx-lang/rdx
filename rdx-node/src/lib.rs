...
pub fn parse(input: String) -> Result<serde_json::Value> {
    ...
    serde_json::to_value(&root).map_err(|e| Error::from_reason(e.to_string()))
}

/// Parse an RDX document with default transforms (auto-slug, table of contents).
#[napi(js_name = "parseWithDefaults")]
pub fn parse_with_defaults(input: String) -> Result<serde_json::Value> {
...
#[napi]
pub fn validate(ast: serde_json::Value, schema: serde_json::Value) -> Result<serde_json::Value> {
    let root: rdx_ast::Root =
        serde_json::from_value(ast).map_err(|e| Error::from_reason(e.to_string()))?;
    let schema: rdx_schema::Schema =
        serde_json::from_value(schema).map_err(|e| Error::from_reason(e.to_string()))?;
...
#[napi(js_name = "collectText")]
pub fn collect_text(ast: serde_json::Value) -> Result<String> {
    let root: rdx_ast::Root =
        serde_json::from_value(ast).map_err(|e| Error::from_reason(e.to_string()))?;
...
#[napi(js_name = "queryAll")]
pub fn query_all(ast: serde_json::Value, node_type: String) -> Result<serde_json::Value> {
    let root: rdx_ast::Root =
        serde_json::from_value(ast).map_err(|e| Error::from_reason(e.to_string()))?;
